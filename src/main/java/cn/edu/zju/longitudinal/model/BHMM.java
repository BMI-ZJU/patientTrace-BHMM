package cn.edu.zju.longitudinal.model;
...
import static cn.edu.zju.longitudinal.Data.*;
...
public class BHMM implements Serializable{
     ...
     * @param file 包含数据文件的根目录
     */
    public void initializeModel(String file) throws IOException {

        int[][][] longitudinalData = loadData(file);
        int[] V = getV();

        int events = longitudinalData[0][0].length;  // 事件的数目
        A = events;
        this.zt = new int[longitudinalData.length][];
        ...
        for (int i=0; i<this.k; i++) {  // todo: 对于categorical 特征，应该是int[1]，仅代表有或无
            for (int a=0; a<events; a++) {
                this.n_zav[i][a] = new int[V[a]];
                this.omega[i][a] = new double[V[a]];
            }
        }

        for (int k=0; k<longitudinalData.length; k++) {
            int lastTopic = -1;
            int currentTopic;
            int[][] content = longitudinalData[k];
            this.zt[k] = new int[longitudinalData[k].length-1];  // -1 因为主题转移的次数比入院次数少1

            for (int i=1; i<content.length; i++) {
                int[] oneDay = content[i];
                ...
                for (int j=0; j<oneDay.length; j++) {
                    int intensity = oneDay[j] - 1;
                    if (intensity < 0) continue;
                    this.n_za[currentTopic][j] += 1;
                    this.za[currentTopic] += 1;
                    this.n_zav[currentTopic][j][intensity] += 1;
                }
    ...
    public void inferenceModel(String file) throws IOException {
        int[][][] longitudinalData = loadData(file);
        Map<Integer, Integer> eAndv = new HashMap<>();

        for (int i=0; i<this.iterations; i++) {
            for (int j=0; j<longitudinalData.length; j++) {
                int[][] content = longitudinalData[j];
                int lastTopic = -1;
                int currentTopic;
                int nextTopic;
                for (int k=1; k<content.length; k++) {
                    ...
                    int[] oneDay = content[k];

                    for (int l=1; l<oneDay.length; l++) {
                        int intensity = oneDay[l] - 1;
                        if (intensity < 0) continue;
                        eAndv.put(l, intensity);
                    }
    ...
    private int sampleTopic(int last_z, int current_z, int next_z, Map<Integer, Integer> eAndv) {
        if (last_z != -1) {
            this.n_zz[last_z][current_z] -= 1;
            this.zz[last_z] -= 1;
        }
        if (next_z != -1) {
            this.n_zz[current_z][next_z] -= 1;
            this.zz[current_z] -= 1;
        }
        for (Map.Entry<Integer, Integer> entry : eAndv.entrySet()) {
            this.n_za[current_z][entry.getKey()] -= 1;
            this.za[current_z] -= 1;
            this.n_zav[current_z][entry.getKey()][entry.getValue()] -= 1;
        }
        ...
        for (int i=0; i<this.k; i++) {
            if (last_z == -1) {
                ...
                p[i] = (this.n_zz[last_z][i] + this.alpha) / (this.zz[last_z] + this.k * this.alpha);
            }
            for (Map.Entry<Integer, Integer> entry : eAndv.entrySet()) {
                int[] av = this.n_zav[i][entry.getKey()];
                int V = av.length;
    ...
    public List<Integer> inferOne(int[][] content) throws IOException {
        ...
        for (int i=1; i<content.length; i++) {
            ...
            for (int k=0; k<this.k; k++) {
                if (lastTopic == -1) {
                    ...
                    p[k] = this.theta[lastTopic][k];
                }
                for (Map.Entry<Integer, Integer> ev : eAndv.entrySet()) {
                    p[k] = p[k] * this.phi[k][ev.getKey()] * this.omega[k][ev.getKey()][ev.getValue()];
                }
            }
    ...
    public static void main(String[] args) throws IOException {
        ...
        bhmm.initializeModel("resources/longitudinal data/feature_df.csv");
        bhmm.saveModel("resources/longitudinal data/result/initializedModel.model");
        System.out.println("Inference model");
        bhmm.inferenceModel("resources/longitudinal data/feature_df.csv");
        ...
        bhmm.saveModel("resources/longitudinal data/result/bhmm_" + k + "_topic.model");
